...
class AnswerRelevanceScore(Transform):
    ...
    def __call__(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        gen_questions = record["gen_questions"].split("\n")
        question = record[self.model_output_key]
        record[self.output_key] = self._get_score(self.embeddings_model, question, gen_questions)
        return record

    @staticmethod
    def _get_score(embeddings_model: ModelRunner, question: str, generated_questions: List[str]) -> float:
        ...
        :param generated_questions: An artificial question generated by judge model based on the answer.
        ...
        gen_question_vectors = []
        for generated_question in generated_questions:
            gen_question_vector = np.asarray(embeddings_model.predict(generated_question))
            gen_question_vectors.append(gen_question_vector)
        norm = np.linalg.norm(gen_question_vectors) * np.linalg.norm(question_vector)
...
class GenerateQuestions(Transform):
    ...
    def __call__(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        record_with_raw_questions = gen_raw_questions(record)
        raw_questions = record_with_raw_questions[RAW_GEN_QUESTIONS].split("\n")
...
class AnswerRelevance(EvalAlgorithmInterface):
    ...
    def _build_pipeline(
        ...
        gen_questions = GenerateQuestions(
            input_key=QUESTION_GENERATION_PROMPT,
            output_key="gen_questions",
            judge_model=judge_model,
        )
        compute_score = AnswerRelevanceScore(embeddings_model=embeddings_model)
        transforms = [gen_question_generation_prompt, gen_questions, compute_score]
