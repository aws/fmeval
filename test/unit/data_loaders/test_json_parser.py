...
class TestJsonParser:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            TestCaseParseColumnFailure(
                result=[[1], 2], error_message="Expected a 2D array", column=DatasetColumns.CONTEXT
            ),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            TestCaseParseColumnFailure(
                result="Not a list", error_message="Expected to find a List", column=DatasetColumns.CONTEXT
            ),
        ],
    )
    ...
    def test_json_parse_dataset_columns_success_json(self, mock_logger, config, dataset):
        ...
        assert cols[DatasetColumns.CONTEXT.value.name] == expected_context
