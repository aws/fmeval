...
from reporting.constants import CENTER, LEFT, RIGHT
...
class TestCell:
    ...
    def test_table_cell_init_success(self, data, headers, table_align, cell_align):
        """
        ...
        """
        table = TableCell(data, headers, table_align=table_align, cell_align=cell_align)
        html = str(table)
        ...
        for header in headers:
            header_cell = f"<th align={cell_align}>{header}</th>"
            assert header_cell in html

        # assert every row in `data` is present in the HTML
        for row in data:
            for elem in row:
                table_cell = f"<td align={cell_align}>{elem}</td>"
