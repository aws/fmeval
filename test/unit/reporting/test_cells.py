...
class TestCell:
    ...
    @pytest.mark.parametrize(
        ...
        [
            TestCaseFigureCellInit(
                args=[],
                expected=FIGURE_CELL_NO_ARGS_STYLE,
            ),
            TestCaseFigureCellInit(
                args=["200", "100", False],
                expected=FIGURE_CELL_WITH_ARGS_STYLE,
            ),
        ],
    )
    def test_figure_cell_init(self, args, expected):
        ...
        # Check if the result contains the expected style
        assert expected in result

        # Check if it contains base64 image data and validate it
        assert "src='data:image/png;base64," in result
        base64_data = result.split("base64,")[1].split("'")[0]
        try:
            base64.b64decode(base64_data)
        except Exception as e:
            pytest.fail(f"Invalid base64 data: {str(e)}")
